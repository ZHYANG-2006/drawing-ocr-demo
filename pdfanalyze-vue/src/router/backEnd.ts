...
export async function setRouters() {
  const { frameInRoutes, frameOutRoutes } =
    await useFrontendMenuStore().getRouter();
  ...
  return routes.map((item: any) => {
    ...
    if (item.is_link) {
      ...
      if (item.is_iframe) {
        item.component = dynamicImport(
          dynamicViewsModules,
          'layout/routerView/iframes',
        );
      } else {
        item.component = dynamicImport(
          dynamicViewsModules,
          'layout/routerView/link',
        );
      }
    } else {
      if (item.is_iframe) {
