...
<script setup lang="jsx">
  ...
  import {
    ...
    getList,
    getTask,
  } from '/@/views/pdfanalyze/stack/api/stackfile/pdfFile.js';
      ...
      title: () => (
        <div style="text-align: center;">
          ...
          <a-input
            placeholder=""
            style="width: 120px; height: 20px;"
            value={filterValue.value.materials}  // 绑定值
            onInput={e => {
              filterValue.value.materials = e.target.materials;
              debouncedFetchPdfFiles();
            }} // 更新 filterValue
            onClick={(e) => e.stopPropagation()}  // 防止点击时触发排序
          />
      ...
      if (response && Array.isArray(response.data)) {
        pdfFiles.value = response.data.map((file) => ({
          ...
          materials: file.materials
            ? file.materials.map(mat => `[${mat.name}]`).join(', ')
            : '',
          queue_order:
            file.queue_order === 0
              ? '已完成'
