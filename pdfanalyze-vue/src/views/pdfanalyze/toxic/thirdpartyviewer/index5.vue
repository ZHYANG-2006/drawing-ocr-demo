...
<script setup>
  ...
  import pdfPath from '/@/assets/SGS_CANAFULL_Conductive fabric adhesive_ TN-B-DJ2-50B HH-5203S HH-5205B gray_ROHS_HF_4P_20201224.pdf';
  import jsonData from '/@/assets/SGS_CANAFULL_Conductive fabric adhesive_ TN-B-DJ2-50B HH-5203S HH-5205B gray_ROHS_HF_4P_20201224.json';
  import {
    getJson,
    getPDF,
  ...
  const loadData = async () => {
    try {
      // 直接从 public 文件夹读取 PDF 文件
      const pdfResponse = await fetch(pdfPath);
      if (!pdfResponse.ok) throw new Error('PDF 文件读取失败');
      pdfFile.value = await pdfResponse.blob(); // 将 PDF 文件转换为 Blob
      // 直接从 public 文件夹读取 JSON 文件
      const jsonResponse = jsonData;
      if (!jsonResponse) throw new Error('JSON 文件读取失败');
      ocrData.value = jsonResponse;
      addUniqueIdToData(jsonResponse[0].content);
      pageIndex.value = buildPageIndex(jsonResponse[0].content);
    } catch (error) {
  ...
  function buildPageIndex(data) {
    ...
    data.forEach((section) => {
      ...
      index[page].push(section);
      if (section.type === 'Table') {
        section.value.forEach((cell) => {
          index[page].push(cell)
        });
      }
    });
    return index;
