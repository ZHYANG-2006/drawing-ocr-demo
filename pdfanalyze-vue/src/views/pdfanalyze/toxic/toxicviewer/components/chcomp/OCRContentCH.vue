<template>
  <div ref="containerRef" class="ocr-content">
    ...
    >
      <h3
        :id="`contentch-${section.type}-${section.id}`"
        :key="`contentch-${section.type}-${section.id}`"
      ...
      >
        {{
          section.has_strike_through || section.has_xbar
            ? section.true_ch_value
            : section.ch_value || ''
        }}
      </h3>
      <div
        v-for="content in section.contents[0]?.elements"
        :id="`contentch-${section.type}-${content.id}`"
        :key="`contentch-${section.type}-${content.id}`"
...
<script setup>
  ...
  import OCRParagraph from './OCRParagraphCH.vue';
  import OCRTable from './OCRTableCH.vue';
  import OCRImage from './OCRImageCH.vue';
  ...
  function scrollToSelectedContent() {
    if (props.selectedContent) {
      const element = document.getElementById(
        `contentch-${props.selectedContent.type}-${props.selectedContent.id}`,
      );
