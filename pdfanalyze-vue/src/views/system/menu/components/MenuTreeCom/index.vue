<template>
  <div>
    <a-input v-model:value="filterVal" placeholder="请输入菜单名称">
      <template #prefix>
        <SearchOutlined />
      </template>
    </a-input>
    <div class="menu-tree-com">
      <div class="mtc-head">
        ...
        <a-tooltip
          placement="right"
          title="1.红色菜单代表状态禁用; 2.添加菜单，如果是目录，组件地址为空即可; 3.添加根节点菜单，父级ID为空即可; 4.支持拖拽菜单;"
        >
      ...
      <div class="mtc-tags">
        ...
        <a-tooltip title="编辑">
          <EditOutlined
            class="mtc-tags-icon"
            @click="handleUpdateMenu('update')"
          />
        </a-tooltip>

        <a-tooltip title="上移">
          <ArrowUpOutlined class="mtc-tags-icon" @click="handleSort('up')" />
        </a-tooltip>
        ...
        <a-tooltip title="删除">
          <DeleteOutlined class="mtc-tags-icon" @click="handleDeleteMenu()" />
        </a-tooltip>
      </div>
    </div>
  </div>
...
<script lang="ts" setup>
  ...
  const emit = defineEmits(['treeClick', 'deleteMenu', 'updateMenu']);
  ...
  const handleUpdateMenu = (type: string) => {
    if (type === 'update' && !treeSelectMenu.value.id) {
      warningNotification('请选择菜单！');
      return;
    }
    console.log('treeselectMenu@');
    console.log(treeSelectMenu.value);
    emit('updateMenu', type, treeSelectMenu.value);
  };
  ...
  const handleDeleteMenu = () => {
    ...
    emit('deleteMenu', treeSelectMenu.value.id, () => {
  ...
  const handleSort = async (type: string) => {
    ...
    const parentList =
      treeData.value.find(
        (node) => node.id === treeSelectNode.value?.parent.key,
      )?.children || [];
    const index = parentList.findIndex(
      (i: DataNode) => i.id === treeSelectMenu.value.id,
    );
    const record = parentList.find(
      (i: DataNode) => i.id === treeSelectMenu.value.id,
    );
    if (type === 'up') {
      if (index === 0) return;
    ...
    if (record) {
      if (type === 'up') {
        await menuMoveUp({ menu_id: record.id });
      } else {
        await menuMoveDown({ menu_id: record.id });
      }
    }
