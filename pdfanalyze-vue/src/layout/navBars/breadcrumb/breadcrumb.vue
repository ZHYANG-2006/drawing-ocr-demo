<template>
  <div v-if="isShowBreadcrumb" class="layout-navbars-breadcrumb">
    ...
    >
      <!-- 通过 v-if 逻辑判断切换图标 -->
      <MenuFoldOutlined v-if="!themeConfig.isCollapse" />
      <MenuUnfoldOutlined v-else />
    </div>
    <a-breadcrumb class="layout-navbars-breadcrumb-hide">
      <a-breadcrumb-item
        v-for="(v, k) in state.breadcrumbList"
        :key="!v.meta.tagsViewName ? v.meta.title : v.meta.tagsViewName"
        :class="{
          'layout-navbars-breadcrumb-span':
            k === state.breadcrumbList.length - 1,
        }"
        @click.prevent="
          k !== state.breadcrumbList.length - 1 && onBreadcrumbClick(v)
        "
      >
        <SvgIcon
...
<script setup lang="ts" name="layoutBreadcrumb">
  ...
  const isShowBreadcrumb = computed(() => {
    ...
    return layout !== 'classic' && layout !== 'transverse' && isBreadcrumb;
  });
  ...
  const onBreadcrumbClick = (v: RouteItem) => {
    ...
    if (redirect) {
      ...
      console.warn('Path is undefined for route:', v);
    }
  };
  // 展开/收起左侧菜单点击
