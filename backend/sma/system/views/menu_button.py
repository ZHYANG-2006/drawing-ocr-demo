...
from sma.system.models.system import MenuButton, RoleMenuButtonPermission
from sma.utils.json_response import DetailResponse, SuccessResponse
...
class MenuButtonViewSet(CustomModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        :return:
        """
        querysettest = self.get_queryset();
    ...
    def batch_create(self, request, *args, **kwargs):
        ...
        result_list = [
            {'menu': menu_obj.id, 'name': '新增', 'value': f'{menu_obj.component_name}:Create',
             'api': f'/api{menu_obj.web_path}/',
             'method': 1},
            {'menu': menu_obj.id, 'name': '删除', 'value': f'{menu_obj.component_name}:Delete',
             'api': f'/api{menu_obj.web_path}/{{id}}/',
             'method': 3},
            {'menu': menu_obj.id, 'name': '修改', 'value': f'{menu_obj.component_name}:Update',
             'api': f'/api{menu_obj.web_path}/{{id}}/',
             'method': 2},
