...
from sma.system.models.csrfile import CSRPdfFile
...
class CSRPdfFileSerializer(CustomModelSerializer):
    """
...
class CSRPdfFileViewSet(CustomModelViewSet):
    """
    PdfFile 的视图集
    继承自 CustomModelViewSet，支持批量删除、过滤查询等
    """
    queryset = CSRPdfFile.objects.all()
    serializer_class = CSRPdfFileSerializer

    # 定义用于创建和更新的序列化器（如果与默认不同）
    create_serializer_class = CSRPdfFileSerializer
    update_serializer_class = CSRPdfFileSerializer
    ...
    def create(self, request, *args, **kwargs):
        ...
        file_name, file_extension = os.path.splitext(file.name)
        if CSRPdfFile.objects.filter(customer_file_name=request.data['customer_file_name'],
                                     customer_file_code=request.data['customer_file_code'],
                                     customer_file_version=request.data['customer_file_version']).exists():
    ...
    def list(self, request, *args, **kwargs):
        ...
        :return:
        """
        queryset = self.filter_queryset(self.get_queryset()).order_by('customer_file_name').order_by('-create_datetime')
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        try:
            ...
            instance = CSRPdfFile.objects.get(pk=pk)
            # 删除数据库记录
            instance.delete()
