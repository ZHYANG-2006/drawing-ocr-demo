...
from sma.system.models.csrfile import CSRJsonTableCell
from sma.system.models.csrfile import CSRPdfFile, CSRJsonFile, CSRJsonSection, CSRJsonContent, CSRJsonParagraph, CSRJsonTable, CSRJsonImage
import threading
...
class CSRFileSerializer(CustomModelSerializer):
    """
...
class CSRFileTaskViewSet(CustomModelViewSet):
    queryset = CSRPdfFile.objects.all()
    serializer_class = CSRFileSerializer

    # 定义用于创建和更新的序列化器（如果与默认不同）
    create_serializer_class = CSRFileSerializer
    update_serializer_class = CSRFileSerializer
    ...
    def process_file(self, pdf_file):
        ...
        try:
            ...
            if response_data:
                ...
                # 指定存储路径
                upload_dir = settings.PATH_JSON_CSRFILE
                ...
                for section_data in response_data:
                    ...
                    json_section = CSRJsonSection.objects.create(
                    ...
                    json_content = CSRJsonContent.objects.create(
                    ...
                    for content_data in contents:
                        ...
                        if content_type == "Paragraph-text" or content_type == "Paragraph":
                            ...
                            if xbars and len(xbars) > 0:
                                ...
                                for xbar_item in xbars:
                                    for content in xbar_item.get("content", []):
                                        ...
                                        if which_word in value_xbar:
                                            value_xbar = value_xbar.replace(modified_word, which_word)
                            CSRJsonParagraph.objects.create(
                            ...
                            json_table = CSRJsonTable.objects.create(
                                content=json_content,
                                page=content_data.get("page", 0),
                            ...
                            for cell in cells_data:
                                ...
                                for span in range(row_span):
                                    ...
                                    order_of_cell += 1
                                    # 创建单元格
                                    CSRJsonTableCell.objects.create(
                                        table=json_table,
                                        page=page,
                                        cell_type=cell_type,
                                        value=value,
    ...
    def schedule_parse(self, request):
        ...
        try:
            pdf_file = CSRPdfFile.objects.get(id=file_id)
        ...
        with parse_lock:
            ...
            max_queue_order = CSRPdfFile.objects.aggregate(max_order=Max('queue_order'))['max_order'] or 0

            # 如果没有文件正在解析，直接将该文件置为解析中
            if not CSRPdfFile.objects.filter(queue_order=1).exists():
    ...
    def start_next_file(self):
        """处理队列中的下一个文件"""
        next_file = CSRPdfFile.objects.filter(queue_order__gt=1).order_by("queue_order").first()
        if next_file:
            # 将下一个文件标记为解析中
