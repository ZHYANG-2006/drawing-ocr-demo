...
def _trim_customer_pn(cp: str) -> str:
    parts = cp.split("-", 2)
    return "-".join(parts[:2]) if len(parts) >= 2 else cp
...
def fetch_jade_rpa_info(mco_pn: str):
    ...
    sql = r"""
            ...
            ORDER BY
                CASE
                    WHEN SUBSTRING_INDEX(MCO_PN, '-', -1) REGEXP '^[0-9]+$'
                    THEN LPAD(SUBSTRING_INDEX(MCO_PN, '-', -1), 10, '0')
                    ELSE CONCAT('Z', SUBSTRING_INDEX(MCO_PN, '-', -1))
                END DESC
            LIMIT 1
        """
